...
legend show
title('$L^{1}$ error of $\phi_{j}^{(r)}$ with respect to the number of iterations, using the HNA iterative method - partial shadowing')
...
xlabel('$x/L_{1}$')
ylabel('$\phi_{1}^{(r)}$')
title('HNA iterative method $\phi_{1}^{(r)}$ - partial shadowing')
legend show
...
xlabel('$x/L_{2}$')
ylabel('$\phi_{2}^{(r)}$')
title('HNA iterative method $\phi_{2}^{(r)}$ - partial shadowing')
legend show
