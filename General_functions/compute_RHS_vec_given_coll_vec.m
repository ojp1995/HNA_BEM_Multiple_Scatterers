function f_ell = compute_RHS_vec_given_coll_vec(Gamma_ell, L_ell, k, d, ...
...
% outer integral.
Psi_ell_r = 2*duidn(Gamma_ell, L_ell, k, d, nq_ell_PIM) ...
    - 2*midpoint_dphikdn_f_diff_screen(k, y1nq_ell_PIM, y2nq_ell_PIM, ...
    h_j_inner, y1nq_j_inner, y2nq_j_inner, phi_j_inner.', norm_j).';

% now we will compute the whole vector
f_ell = incident(k, theta, x1s_ell, x2s_ell) ...
    - midpoint_hankel_f_diff_screen(k, x1s_ell, x2s_ell, y1nq_j, ...
    y2nq_j, h_j_outer, phi_j_outer.') ...
    - PIM_int_hankel_f(k, s_ell, h_ell_outer_PIM, nq_ell_PIM, ...
    Psi_ell_r, tq_ell_PIM, C1, C2); 
