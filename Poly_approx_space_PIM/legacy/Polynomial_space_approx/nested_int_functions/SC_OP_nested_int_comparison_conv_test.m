...
%     1 and then on screen 2
    ...
    [err_normG1_it, err_normG2_it, aj_1_r, aj_2_r] = nested_int_it_conv_test_fixed_dof( G1, G2, k, C1, C2, theta, C_wl(j), R_it, R_true); 

    
    % Now computing errors between the two, SC true same grid
    err_norm_SC_true_samegrid(j, 1) = norm(phi1_0 - aj_1_r(:, 1), 1)/norm(phi1_0, 1);
    err_norm_SC_true_samegrid(j, 2) = norm(phi2_1 - aj_2_r(:, 1), 1)/norm(phi2_1, 1);
    err_norm_SC_true_samegrid(j, 3) = norm(phi1_2 - aj_1_r(:, 2), 1)/norm(phi1_2, 1);
    err_norm_SC_true_samegrid(j, 4) = norm(phi2_3 - aj_2_r(:, 2), 1)/norm(phi2_3, 1);
    

    err_norm_eachhalftrue(j, 1) = norm(phi1_0 - aj_1_r(:, 1), 1)/(norm(phi1_0, 1)/2+ norm(aj_1_r(:, 1), 1)/2 );
    err_norm_eachhalftrue(j, 2) = norm(phi2_1 - aj_2_r(:, 1), 1)/(norm(phi2_1, 1)/2+ norm(aj_2_r(:, 1), 1)/2 );
    err_norm_eachhalftrue(j, 3) = norm(phi1_2 - aj_1_r(:, 2), 1)/(norm(phi1_2, 1)/2+ norm(aj_1_r(:, 2), 1)/2 );
    err_norm_eachhalftrue(j, 4) = norm(phi2_3 - aj_2_r(:, 2), 1)/(norm(phi2_3, 1)/2+ norm(aj_2_r(:, 1), 1)/2 );

    %plotting
    figure(1)
    plot(((1:N1) - 0.5)/N1, real(aj_1_r(:, 1)), 'DisplayName', sprintf('OC dof= %g', 1/C_wl(j)))
    hold on
    plot(((1:N1) - 0.5)/N1, real(phi1_0), 'DisplayName', sprintf('SC dof= %g', 1/C_wl(j)))
    figure(2)
    plot(((1:N2) - 0.5)/N2, real(aj_2_r(:, 1)), 'DisplayName', sprintf('OC dof= %g', 1/C_wl(j)))
    hold on
    plot(((1:N2) - 0.5)/N2, real(phi2_1), 'DisplayName', sprintf('SC dof= %g', 1/C_wl(j)))
    
    figure(3)
    plot(((1:N1) - 0.5)/N1, aj_1_r(:, 2), 'DisplayName', sprintf('OC dof= %g', 1/C_wl(j)))
    hold on
    plot(((1:N1) - 0.5)/N1, real(phi1_2), 'DisplayName', sprintf('SC dof= %g', 1/C_wl(j)))
    figure(4)
    plot(((1:N2) - 0.5)/N2, aj_2_r(:, 2), 'DisplayName', sprintf('OC dof= %g', 1/C_wl(j)))
    hold on
