% test 5a - near trapping, parralell screens
...
it_of_interest = [R_max-5000, R_max - 1000, R_max];
figure()
...
xlabel('$x/L_{1}$')
ylabel('$\phi_{1}^{(r)}$')
title('PC iterative approximation to $\phi_{1}$ - parallel screens')
xlim([-0.05 1.05])
...
xlabel('$x/L_{2}$')
ylabel('$\phi_{2}^{(r)}$')
title('PC iterative approximation to $\phi_{2}$ - parallel screens')
xlim([-0.05 1.05])
...
title('$L^{1}$ error of $\phi_{j}^{(r)}$ with respect to the number of iterations, using the PC iterative method - parallel screens')
