function dudn = bem2BS(x1,y1,x2,y2,h1,h2,phi1,k,d)
%
% function dudn = bem2BS(x,y,h,k,d)
%
% This is a hacked version of bem2, altered so that the incident field also
% includes the beam source from screen 1.
%
...
%
N = length(x2); % Number of boundary elements on screen 2
...
gr = (1:Nsing)-0.5; 
b = (ui(x2,y2,k,d(1),d(2)) + us(x2,y2,k,x1,y1,h1,phi1)).'; % Evaluating the RHS of the BIE (1) at the collocation points om Gamma.
for n = 1:N
    % Evaluating the nth row of A
    h_small = 0.5*h2(n)/Nsing; % Stepsize for composite midpoint rule evaluation of the diagonal element
    A(n,n) = (i/2)*h_small*sum(besselh(0,k*gr*h_small)); % First the entry on the diagonal  
    select = (1:N)~=n; % Now the off-diagonal elements
    A(select,n) = (i*h2(n)/4)*besselh(0, k*sqrt( (x2(select)-x2(n)).^2 + (y2(select)-y2(n)).^2 ) );
end
dudn = A\b; 